<div class="product-one" id="prod-block">
    <div class="le-prod_block">
        <p class="big-text_product">ECOMET TOP Кордун</p>
        <p class="small-text-product">Устойчив к автомобильным жидкостям и химическим веществам. Представляет собой однокомпонентную систему</p>
        <div class="le-prod_bl_bottom">
            <a class="but-prod_pocr" href="">
                <p class="" id="">Подробнее</p>
            </a>
            <div class="le-block_color">
                <p>Цвет:</p>
                <div class="bl-color"></div>
            </div>
        </div>
    </div>
    <div class="ri-prod_block">
        <img class="product_img" src="../img/pocr-ban.svg" alt="" />
    </div>
</div>